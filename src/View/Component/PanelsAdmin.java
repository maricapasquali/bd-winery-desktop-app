...
public class PanelsAdmin extends PanelsEmployee {
	...
	private static JPanel caskPane() {
		final JPanel pane = Components.createPaneBorder("Aggiungere Botte");
		...
		// Button aggiungi
		final JPanel paneAdd = Components.createPaneFlow();
		final JButton button = Components.createButton("Inserisci Uva");
		paneAdd.add(button);
		button.addActionListener(e -> {
			boolean success = false;
			try {
	...
	private static JPanel harvesterPane() {
		...
		final JPanel pane = Components.createPaneBorder("Aggiungere Vendemmiatrice");
